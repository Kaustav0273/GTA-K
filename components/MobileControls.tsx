...
const MobileControls: React.FC<MobileControlsProps> = ({ isDriving, controlStyle }) => {
    ...
    const Button = ({ code, icon, className, color = 'bg-gray-800/50', label }: { code: string, icon?: string, className?: string, color?: string, label?: string }) => (
        <button
    ...
    const Joystick = () => {
        ...
        const handleTouch = (clientX: number, clientY: number, end: boolean = false) => {
            ...
            if (isDriving) {
                // Driving: Only Steering (A/D) on Joystick if pedals are separate?
                // The prompt asked for "pedals for car driving". Typically this means steering is on left (joystick/buttons) and gas/brake on right.
                // So Joystick here maps to A/D
            ...
            <div className="absolute bottom-6 left-6 w-48 h-48 pointer-events-auto flex items-end justify-start">
                 {controlStyle === 'JOYSTICK' ? (
                     ...
                     <div className="relative w-full h-full">
                         {isDriving ? (
                            // Driving D-PAD (Only Left/Right needed usually if gas is on right, but keep WASD structure just in case or simplified L/R)
                            <>
                                <Button code="KeyA" icon="fa-chevron-left" className="absolute left-0 top-1/2 -translate-y-1/2 w-20 h-20" />
                                <Button code="KeyD" icon="fa-chevron-right" className="absolute left-24 top-1/2 -translate-y-1/2 w-20 h-20" />
                            </>
                         ) : (
                            // Walking D-PAD
                            <>
                                <Button code="KeyW" icon="fa-chevron-up" className="absolute top-0 left-1/2 -translate-x-1/2 w-16 h-16" />
                                <Button code="KeyS" icon="fa-chevron-down" className="absolute bottom-0 left-1/2 -translate-x-1/2 w-16 h-16" />
                                <Button code="KeyA" icon="fa-chevron-left" className="absolute left-0 top-1/2 -translate-y-1/2 w-16 h-16" />
                                <Button code="KeyD" icon="fa-chevron-right" className="absolute right-0 top-1/2 -translate-y-1/2 w-16 h-16" />
            ...
            <div className="absolute bottom-6 right-6 w-64 h-64 pointer-events-auto">
                 <div className="relative w-full h-full">
                      {isDriving ? (
                          <>
                            {/* Exit Car */}
                            <Button code="KeyF" icon="fa-person-through-window" className="absolute top-0 right-2 w-12 h-12 bg-blue-600/50" />
                            
                            {/* Handbrake */}
                            <Button code="Space" icon="fa-ban" className="absolute top-16 right-24 w-14 h-14 bg-red-800/50" />

                            {/* Pedals */}
                            {/* Brake - Wide, lower left */}
                            <Button code="KeyS" label="" className="absolute bottom-0 right-28 w-20 h-16 bg-gray-600/80 rounded-lg border-b-4 border-black/50 active:border-b-0 active:translate-y-1" />
                            
                            {/* Gas - Tall, far right */}
                            <Button code="KeyW" label="" className="absolute bottom-0 right-0 w-20 h-32 bg-gray-600/80 rounded-lg border-b-4 border-black/50 active:border-b-0 active:translate-y-1" />
                          ...
                          <>
                            {/* Sprint */}
                            <Button code="ShiftLeft" icon="fa-running" className="absolute bottom-32 right-0 w-16 h-16 bg-yellow-600/50" />
                            
                            {/* Enter/Exit Car */}
                            <Button code="KeyF" icon="fa-car" className="absolute top-0 right-0 w-14 h-14 bg-blue-600/50" />
                            
                            {/* Action / Punch / Shoot */}
                            <Button code="Space" icon="fa-hand-fist" className="absolute bottom-0 right-0 w-24 h-24 bg-red-600/50 text-3xl" />
                          </>
                      )}
                 </div>
            </div>
